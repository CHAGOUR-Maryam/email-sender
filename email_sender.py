...
if cf.log_in() == True:
    ...
    EMAIL_ADDRESS = st.text_input("sender email", label_visibility="collapsed", value="maryamchagou@gmail.com ")

    st.markdown("**Password:  :red[*]**")
    PASSWORD = st.text_input("password",placeholder="onilgfjnlbxywzpm",value="onilgfjnlbxywzpm", label_visibility="collapsed")

    st.divider()

    st.markdown("**Receiver emails:** (Enter the Emails seperated by a **LIGNE**)  **:red[*]**")
    EMAILS_TO = st.text_area("receiver email", label_visibility="collapsed", value="maryamchagou@gmail.com nkharmoum@gmail.com ")


    st.markdown("**CC :**")
    CCS = st.text_area("copy", label_visibility="collapsed", value="maryam.chagour@uit.ac.ma n.kharmoum@cnrst.ma")

    st.markdown("**BCC :**")
    BCCS = st.text_area("bcopy", label_visibility="collapsed", value="maryam.chagour@uit.ac.ma n.kharmoum@cnrst.ma")
    ...
    if st.button('SEND'):
        ...
        for email_to in EMAILS_TO:
            try:
                ...
                msg['To'] = email_to
                msg['Subject'] = SUBJECT
                msg['Cc'] = ','.join(CCS) #normally we should have a list of emails seperated with comma ,
                msg['Bcc'] = ','.join(BCCS)
                msg.set_content(EMAIL_CONTENT)
                emails_to_cc = [email_to]
                emails_to_cc.extend(CCS)
                emails_to_cc.extend(BCCS)
                #rcpt = CC.split(",") + [email_to]
